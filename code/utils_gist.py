...
class GistUtils:
    def __init__(self, n_resize=128, n_w=5, ln_orientation=[5, 5, 5, 5], n_block_num=4, n_prefilt=4):
...
#        np_gf = np.fft.fftshift(np.exp( -(np_fx **2 + np_fy **2)/(n_s1 ** 2)))
    ...
    def __down_sampling(self, np_img):
        np_index = np.linspace(0, self.n_resize, self.n_block_num + 1, dtype = np.int64)
        ln_data = []
