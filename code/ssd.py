...
def com_self_similarities(out_img_path, img_path, region_size, patch_size, bin_size, strides, gap):
    '''it could be changed to smaller size of the image but a bigger channel

    :param src_image:
    :param region_size:needs to be odd
    :param patch_size: needs to be odd
    :param bin:
    :param gap:
    :return:
    '''
    ...
    size = image.shape
    #print("size", size)
    out_img_height = (size[0] - region_size[0])//strides[0] + 1
    out_img_width = (size[1] - region_size[1])//strides[1] + 1
    self_similarities = np.zeros((out_img_height, out_img_width, bin_size[0]*bin_size[1]))
    #print(self_similarities.shape)
    center_patch = [patch_size[0]//2, patch_size[1]//2]

    for row in range(0, out_img_height*strides[0], strides[0]):
        for col in range(0, out_img_width*strides[1], strides[1]):
            #print("row, col", row, col)
            mid_point = [(row+region_size[0])//2, (col+region_size[1])//2]
            region = image[row:row+region_size[0], col:col+region_size[1]]
            patch = image[mid_point[0]-center_patch[0]:mid_point[0]+(patch_size[0]-center_patch[0]), mid_point[1]-center_patch[1]:mid_point[1]+(patch_size[1]-center_patch[1])]
            ssd_region = cal_ssd(patch, region, gap)
            #print("SSD_region", ssd_region.shape)
            radius, angle = cart2polar(ssd_region.shape)
            bins = get_bin(radius, angle, ssd_region.shape, bin_size)
            vec = get_self_sim_vec(ssd_region, bins)
            LSSD = mapminmax(vec, 0, 1).flatten()
            self_similarities[row//strides[0], col//strides[1], :] = LSSD
    print(self_similarities.shape)
    outimg = out_img_path
    normalized_array = ((self_similarities - self_similarities.min()) / (self_similarities.max() - self_similarities.min()) * 255).astype(np.uint8)
    image = Image.fromarray(normalized_array)
    image.save(outimg)

def feature_maps(srcdir, outdir):
    region_size = [1, 15]
    patch_size = [1, 3]
    bin_size = [1, 3]
    strides = region_size
            ...
            if os.listdir(subdirectory_path):
                ...
                for filename in os.listdir(subdirectory_path):
                    ...
                    if Path(source_filepath).exists():
                        com_self_similarities(destination_path, source_filepath, region_size, patch_size, bin_size, strides, gap)


def generate_feature_maps():
    srcdir=r'./WindowsGreyImage'
    outdir = r'./parallel_windows_maps'
    feature_maps(srcdir, outdir)
...
def parallel_windows_maps(srcdir, outdir):
    ...
    execution_time = execution_time/25739
    print(execution_time)

parallel_windows_maps(r'./com_fam_dir_win', r'./WindowsMalware')
